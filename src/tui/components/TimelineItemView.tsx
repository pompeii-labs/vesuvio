...
export function TimelineItemView({ item, width, prevType }: Props) {
    // Smart spacing: add gap between different "blocks" but keep tools tight
    const needsGap =
        prevType &&
        !(
            // Keep tools tight together
            (
                (prevType === 'tool_start' && item.type === 'tool_start') ||
                (prevType === 'tool_start' && item.type === 'tool_end') ||
                // Keep thinking right after user
                (prevType === 'user' && item.type === 'thinking')
            )
        );
